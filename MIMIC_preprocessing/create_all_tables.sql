...
-- change the paths to those in your local computer.
...
-- renaming some of the variables so that they are equivalent to those in MIMIC
\copy (select subject_id as uniquepid, hadm_id as patienthealthsystemstayid, stay_id as patientunitstayid, hospital_expire_flag as actualhospitalmortality, los as actualiculos from ld_labels as l where l.stay_id in (select * from ld_timeseries_patients) order by l.stay_id) to '/MIMIC_data/labels.csv' with csv header
\copy (select * from ld_flat as f where f.patientunitstayid in (select * from ld_timeseries_patients) order by f.patientunitstayid) to '/MIMIC_data/flat_features.csv' with csv header
\copy (select * from ld_timeserieslab as tl order by tl.patientunitstayid, tl.labresultoffset) to '/MIMIC_data/timeserieslab.csv' with csv header
\copy (select * from ld_timeseries as t order by t.patientunitstayid, t.chartoffset) to '/MIMIC_data/timeseries.csv' with csv header