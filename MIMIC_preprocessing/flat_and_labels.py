...
def flat_and_labels_main(MIMIC_path):
    ...
    if os.path.exists(MIMIC_path + 'stays.txt'):
        ...
        id_column = 'patient' if 'patient' in ts_patients.columns else 'patientunitstayid'
        ts_patients = ts_patients[id_column].unique().tolist()
        
        with open(MIMIC_path + 'stays.txt', 'w') as f:
            for patient in ts_patients:
                f.write("%s\n" % patient)
    
    ts_patients = [p for p in ts_patients if p in flat.index and p in labels.index]
    flat = flat.loc[ts_patients].copy()
    labels = labels.loc[ts_patients].copy()

    print(f"Final number of patients with all data: {len(ts_patients)}")
