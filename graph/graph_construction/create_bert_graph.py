...
import torch
import argparse
from graph.graph_construction.create_graph import get_device_and_dtype
import json


def make_graph_bert(bert, batch_size=1000, debug=False, k=3, mode='k_closest', eps=0.005):
    print('==> Getting edges')
    no_pts = 94428 if debug is False else 10000
    edges = sparse.lil_matrix((no_pts, no_pts), dtype=np.uint8)
    # Use float32 instead of float16
    edges_val = sparse.lil_matrix((no_pts, no_pts), dtype=np.float32)
    ...
    for i, batch in enumerate(torch.split(bert, batch_size, dim=0)):
        ...
        batch = batch.float()  # or batch.to(dtype=torch.float32)
        bert = bert.float()    # or bert.to(dtype=torch.float32)
