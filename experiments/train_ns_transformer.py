"""
Main file for training Transformers
"""
import os
...
from src.models.transformer import TransformerLOSNet
...
from src.config.args import add_configs, init_transformergnn_args
...
class TransformerModel(pl.LightningModule):
    """
    Node sampling Transformer model
    """
    def __init__(self, config, dataset, train_loader, subgraph_loader, eval_split='test', get_transformer_out=False, get_logits=False):
        super().__init__()
        self.config = config
        ...
        self.get_transformer_out = get_transformer_out
        ...
        self.net = TransformerLOSNet(config)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        all_out = torch.cat(transformer_outs, dim=0)
    ...
    def test_step(self, batch, batch_idx):
        ...
        """
        if self.get_transformer_out or self.get_logits:
            transformer_outs = []
            for inputs, labels, ids in tqdm(self.ts_loader):
                seq, flat = inputs
                if self.get_transformer_out:
                    hid = self.net.forward_to_embedding(seq.to(self.device), flat=flat.to(self.device))
                    transformer_outs.append(hid)
                else:
                    out = self.net.forward(seq.to(self.device), flat=flat.to(self.device))
                    transformer_outs.append(out)
            out = torch.cat(transformer_outs, dim=0)
            return {'hid': out}
    ...
    def load_model(log_dir, **hparams):
        ...
        model = TransformerModel.load_from_checkpoint(checkpoint_path=str(model_path), **args)
...
def get_data(config, us=None, vs=None):
    ...
    config['trans_indim'] = dataset.x_dim  # Update for transformer
...
def main(config):
    """
    Main function for training Transformers.
    ...
    model = TransformerModel(config, dataset, train_loader, subgraph_loader)
...
def main_test(hparams, path_results=None):
    ...
    model, config, dataset, train_loader, subgraph_loader = TransformerModel.load_model(log_dir, multi_gpu=hparams['multi_gpu'], num_workers=hparams['num_workers'])
    trainer = pl.Trainer(
        gpus=hparams['gpus'],
        logger=None,
...
def main_forward_pass(hparams):
    ...
    """
    log_dir = hparams['load']
    model, config, dataset, train_loader, subgraph_loader = TransformerModel.load_model(log_dir, \
        data_dir=hparams['data_dir'], multi_gpu=hparams['multi_gpu'], num_workers=hparams['num_workers'])
    if hparams['fp_transformer']:
        model.get_transformer_out = True
    else:
    ...
    if hparams['fp_transformer']:
        out_path = Path(log_dir) / 'transformer_embeddings.npy'
    else:
        out_path = Path(log_dir) / 'transformer_logits.npy'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fp_transformer', action='store_true', help='forward pass to get transformer embeddings')
    ...
    if config['fp_transformer'] or config['fp_logits']:
