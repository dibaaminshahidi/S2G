...
from src.config.hyperparameters.search import main_tune
from src.config.args import init_lstmgnn_args, add_tune_params, add_configs
...
class los_TuneReportCallback(Callback):
    def on_validation_end(self, trainer, pl_module):
        tune.report(
            ...
            mad=trainer.callback_metrics['mad'].item(),
            mse=trainer.callback_metrics['mse'].item(),
            mape=trainer.callback_metrics['mape'].item(),
            msle=trainer.callback_metrics['msle'].item(),
            r2=trainer.callback_metrics['r2'].item(),
            kappa=trainer.callback_metrics['kappa'].item()
        )

def main_train(config):
    import pytorch_lightning.trainer.connectors.slurm_connector as slurm_module
    slurm_module.SLURMConnector.register_slurm_signal_handlers = lambda *args, **kwargs: None
